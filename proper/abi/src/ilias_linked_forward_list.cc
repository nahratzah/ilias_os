...
auto basic_linked_forward_list::unlink(element* e) noexcept -> element* {
  ...
  if (head_ == e) return unlink_front();

  iterator pred;
  for (pred = begin(); pred->succ_ != e; ++pred);
  pred->succ_ = exchange(e->succ_, nullptr);
  return e;
}

auto basic_linked_forward_list::splice_front(
    basic_linked_forward_list& o) noexcept -> void {
  using _namespace(std)::exchange;
  ...
  if (!o.empty()) {
    if (head_ == nullptr) {
      head_ = exchange(o.head_, nullptr);
    } else {
      element*& o_e_ptr = o.find_succ_for_(nullptr);
      o_e_ptr = exchange(head_, exchange(o.head_, nullptr));
    }
  }
}
...
auto basic_linked_forward_list::splice_front(
  ...
  if (o_b != o_e) {
    ...
    if (o_e.elem_ == nullptr && head_ == nullptr) {
      head_ = exchange(o_b_ptr, nullptr);
    } else {
      /* Find succ_ pointing at o_e.elem_. */
      element*& o_e_ptr = o.find_succ_for_(o_e.elem_);

      o_b_ptr = o_e.elem_;  // Unlink [o_b..o_e)
      o_e_ptr = exchange(head_, o_b.elem_);  // Link [o_b..o_e) at head_.
    }
  }
}
...
auto basic_linked_forward_list::splice_after(
  ...
  if (!o.empty()) {
    if (head_ == nullptr) {
      ...
      o_end_ptr = exchange(i->succ_, exchange(o.head_, nullptr));
    }
  }
}
