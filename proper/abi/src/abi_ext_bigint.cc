...
unsigned char subc(unsigned char x, unsigned char y,
                   unsigned char carry_in, unsigned char* carry_out)
    noexcept {
  return __builtin_subcb(x, y, carry_in, carry_out);
}

unsigned short subc(unsigned short x, unsigned short y,
                    unsigned short carry_in, unsigned short* carry_out)
    noexcept {
  return __builtin_subcs(x, y, carry_in, carry_out);
}

unsigned int subc(unsigned int x, unsigned int y,
                  unsigned int carry_in, unsigned int* carry_out)
    noexcept {
  return __builtin_subc(x, y, carry_in, carry_out);
}

unsigned long subc(unsigned long x, unsigned long y,
                   unsigned long carry_in, unsigned long* carry_out)
    noexcept {
  return __builtin_subcl(x, y, carry_in, carry_out);
}

unsigned long long subc(unsigned long long x, unsigned long long y,
                        unsigned long long carry_in,
                        unsigned long long* carry_out)
    noexcept {
...
bool umul_overflow(unsigned char x, unsigned char y,
                   unsigned char* result) noexcept {
  using int_t = unsigned int;
  int_t m = int_t(x) * int_t(y);
  *result = m;
  return (m > std::numeric_limits<unsigned char>::max());
}

bool umul_overflow(unsigned short x, unsigned short y,
                   unsigned short* result) noexcept {
  using int_t = unsigned int;
  int_t m = int_t(x) * int_t(y);
  *result = m;
  return (m > std::numeric_limits<unsigned short>::max());
}

bool umul_overflow(unsigned int x, unsigned int y,
                   unsigned int* result) noexcept {
  return __builtin_umul_overflow(x, y, result);
}

bool umul_overflow(unsigned long x, unsigned long y,
                   unsigned long* result) noexcept {
  return __builtin_umull_overflow(x, y, result);
}

bool umul_overflow(unsigned long long x, unsigned long long y,
