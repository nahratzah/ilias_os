...
int __kernel_rem_pio2(double* x, double* y, int e0, int nx, int prec) noexcept {
  static constexpr array<double, 8> PIo2{{
  ...
  static constexpr array<int, 4> init_jk{{ 3, 4, 4, 6 }}; /* initial value for jk */
  static constexpr double zero = 0.0;
  static constexpr double one = 1.0;
  static constexpr double two24 = 1.67772160000000000000e+07;  /* 0x41700000, 0x00000000 */
  static constexpr double twon24 = 5.96046447753906250000e-08;  /* 0x3E700000, 0x00000000 */
  ...
  for (int32_t i = 0, j = jv - jx; i <= m; i++, j++)
...
recompute:
  ...
  array<int32_t, 20> iq;
  double z = q[jz];
  for (int32_t i = 0, j = jz; j > 0; i++, j--) {
    double fw = static_cast<double>(static_cast<int32_t>(twon24 * z));
  ...
  if (ih > 0) {  /* q > 0.5 */
    n += 1;
    int32_t carry = 0;
    for (int32_t i = 0; i < jz; i++) {  /* compute 1-q */
  ...
  if (z == zero) {
    int32_t j = 0;
    for (int32_t i = jz - 1; i >= jk; i--) j |= iq[i];
    if (j == 0) {  /* need recomputation */
      int32_t k;
      for (k = 1; iq[jk - k] == 0; k++);  /* k = no. of terms needed */

      for (int32_t i = jz + 1; i <= jz + k; i++) {  /* add q[jz+1] to q[jz+k] */
  ...
  if (z == 0.0) {
    ...
    z = scalbn(z, -q0);
    if (z >= two24) {
      const double fw = static_cast<double>(static_cast<int32_t>(twon24 * z));
  ...
  case 3:  /* painful */
    {
      for (int32_t i = jz; i > 0; i--) {
        const double fw = fq[i - 1] + fq[i];
        fq[i] += fq[i - 1] - fw;
        fq[i - 1] = fw;
      }
      for (int32_t i = jz; i > 1; i--) {
        const double fw = fq[i - 1] + fq[i];
        fq[i] += fq[i - 1] - fw;
        fq[i - 1] = fw;
      }
