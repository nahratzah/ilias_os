...
void __kernel_sincos( double x, double y, int iy, double* k_s, double* k_c )
    noexcept {
  ...
  static constexpr double S1   = -1.66666666666666324348e-01;  /* 0xBFC55555, 0x55555549 */
  static constexpr double S2   =  8.33333333332248946124e-03;  /* 0x3F811111, 0x1110F8A6 */
  static constexpr double S3   = -1.98412698298579493134e-04;  /* 0xBF2A01A0, 0x19C161D5 */
  static constexpr double S4   =  2.75573137070700676789e-06;  /* 0x3EC71DE3, 0x57B1FE7D */
  static constexpr double S5   = -2.50507602534068634195e-08;  /* 0xBE5AE5E6, 0x8A2B9CEB */
  static constexpr double S6   =  1.58969099521155010221e-10;  /* 0x3DE5D93A, 0x5ACFD57C */
  static constexpr double C1   =  4.16666666666666019037e-02;  /* 0x3FA55555, 0x5555554C */
  static constexpr double C2   = -1.38888888888741095749e-03;  /* 0xBF56C16C, 0x16C15177 */
  static constexpr double C3   =  2.48015872894767294178e-05;  /* 0x3EFA01A0, 0x19CB1590 */
  static constexpr double C4   = -2.75573143513906633035e-07;  /* 0xBE927E4F, 0x809C52AD */
  static constexpr double C5   =  2.08757232129817482790e-09;  /* 0x3E21EE9E, 0xBDB4B1C4 */
  static constexpr double C6   = -1.13596475577881948265e-11;  /* 0xBDA8FAE9, 0xBE8838D4 */
...
void sincos(double x, double* s, double* c) noexcept {
  using impl::__kernel_sincos;
  using impl::__ieee754_rem_pio2;

  /* Store high word of x in ix */
  int32_t ix = get<0>(extract_words(x)) & 0x7fffffff;
  ...
  if (ix <= 0x3fe921fb) {
    ...
    *c = x - x;
    ...
    array<double, 2> y;
    int32_t n = __ieee754_rem_pio2(x, y.data());
    double k_c, k_s;
