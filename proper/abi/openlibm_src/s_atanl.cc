...
long double atanl(long double x) noexcept {
  ...
  const uint16_t expt = expsign & 0x7fff;
  if (expt >= ATAN_CONST) {  /* if |x| is large, atan(x)~=pi/2 */
    mask_nbit_l(u);
    if (expt == BIAS + LDBL_MAX_EXP && (u.manh | u.manl) != 0)
      return x + x;  /* NaN */
    if (expsign > 0)
  ...
  const int32_t expman = (expt << 8) | ((u.manh >> (MANH_SIZE - 9)) & 0xff);
  int id;
  ...
  long double z = x * x;
  const long double w = z * z;
  /* break sum aT[i]z**(i+1) into odd and even poly */
  const long double s1 = z * T_even(w);
  const long double s2 = w * T_odd(w);
  if (id < 0) {
    return x - x * (s1 + s2);
  } else {
