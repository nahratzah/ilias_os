...
static constexpr long double tiny = 1e-4931L;
static constexpr long double half = 0.5L;
static constexpr long double one  = 1.0L;
static constexpr long double two  = 2.0L;
/* c = (float)0.84506291151 */
static constexpr long double erx = 0.845062911510467529296875L;
/*
 * Coefficients for approximation to  erf on [0,0.84375]
 */
/* 2/sqrt(pi) - 1 */
static constexpr long double efx = 1.2837916709551257389615890312154517168810E-1L;
/* 8 * (2/sqrt(pi) - 1) */
static constexpr long double efx8 = 1.0270333367641005911692712249723613735048E0L;

static constexpr array<long double, 6> pp{{
...
static constexpr array<long double, 6> qq{{
...
constexpr array<long double, 8> pa{{
...
constexpr array<long double, 7> qa{{
...
constexpr array<long double, 9> ra{{
...
constexpr array<long double, 9> sa{{
...
constexpr array<long double, 8> rb{{
...
constexpr array<long double, 7> sb{{
...
constexpr array<long double, 6> rc{{
...
constexpr array<long double, 5> sc{{
...
long double erfl(long double x) noexcept {
  ...
  if (ix >= 0x7fff) {  /* erf(nan)=nan */
    const int32_t i = ((se & 0xffff) >> 15) << 1;
    return static_cast<long double>(1 - i) +
           one / x;  /* erf(+-inf)=+-1 */
  }
  ...
  if (ix < 0x3ffed800) {  /* |x|<0.84375 */
    ...
    const long double r = pp[0] + z * (pp[1] + z * (pp[2] + z * (pp[3] + z * (pp[4] + z * pp[5]))));
    const long double s = qq[0] + z * (qq[1] + z * (qq[2] + z * (qq[3] + z * (qq[4] + z * (qq[5] + z)))));
    const long double y = r / s;
    return x + x * y;
  }
  if (ix < 0x3fffa000 /* 1.25 */) {  /* 0.84375 <= |x| < 1.25 */
    const long double s = fabsl(x) - one;
    const long double P = pa[0] + s * (pa[1] + s * (pa[2] + s * (pa[3] + s * (pa[4] + s * (pa[5] + s * (pa[6] + s * pa[7]))))));
    const long double Q = qa[0] + s * (qa[1] + s * (qa[2] + s * (qa[3] + s * (qa[4] + s * (qa[5] + s * (qa[6] + s))))));
    if ((se & 0x8000) == 0)
      return erx + P / Q;
    else
      return -erx - P / Q;
  }
  if (ix >= 0x4001d555 /* 6.6666259765625 */) {  /* inf>|x|>=6.666 */
    if ((se & 0x8000) == 0)
      return one - tiny;
    else
      return tiny - one;
  ...
  const auto x_words = get_ldouble_words(x);
  const long double z = set_ldouble_words(get<0>(x_words), get<1>(x_words), 0);
  const long double r = expl(-z * z - 0.5625) * expl((z - x) * (z + x) + R / S);
  if ((se & 0x8000) == 0)
    return one - r / x;
  else
    return r / x - one;
...
long double erfcl(long double x) noexcept {
  ...
  if (ix >= 0x7fff) {  /* erfc(nan)=nan */
    /* erfc(+-inf)=0,2 */
    return static_cast<long double>(((se & 0xffff) >> 15) << 1) +
           one / x;
  }
  ...
  if (ix < 0x3ffed800) {  /* |x|<0.84375 */
    if (ix < 0x3fbe0000)  /* |x|<2**-65 */
      return one - x;
    const long double z = x * x;
    long double r = pp[0] + z * (pp[1] + z * (pp[2] + z * (pp[3] + z * (pp[4] + z * pp[5]))));
    const long double s = qq[0] + z * (qq[1] + z * (qq[2] + z * (qq[3] + z * (qq[4] + z * (qq[5] + z)))));
    const long double y = r / s;
  ...
  if (ix < 0x3fffa000 /* 1.25 */) {  /* 0.84375 <= |x| < 1.25 */
    ...
    const long double Q = qa[0] + s * (qa[1] + s * (qa[2] + s * (qa[3] + s * (qa[4] + s * (qa[5] + s * (qa[6] + s))))));
    if ((se & 0x8000) == 0) {
      const long double z = one - erx;
      return z - P / Q;
    } else {
      const long double z = erx + P / Q;
      return one + z;
    }
  }
  if (ix < 0x4005d600 /* 107 */) {  /* |x|<107 */
    x = fabsl(x);
    const long double s = one / (x * x);
    long double R, S;
    if (ix < 0x4000b6db /* 2.85711669921875 */) {  /* |x| < 1/.35 ~ 2.857143 */
      R = ra[0] + s * (ra[1] + s * (ra[2] + s * (ra[3] + s * (ra[4] + s * (ra[5] + s * (ra[6] + s * (ra[7] + s * ra[8])))))));
      S = sa[0] + s * (sa[1] + s * (sa[2] + s * (sa[3] + s * (sa[4] + s * (sa[5] + s * (sa[6] + s * (sa[7] + s * (sa[8] + s))))))));
    } else if (ix < 0x4001d555 /* 6.6666259765625 */) {  /* 6.666 > |x| >= 1/.35 ~ 2.857143 */
      R = rb[0] + s * (rb[1] + s * (rb[2] + s * (rb[3] + s * (rb[4] + s * (rb[5] + s * (rb[6] + s * rb[7]))))));
      S = sb[0] + s * (sb[1] + s * (sb[2] + s * (sb[3] + s * (sb[4] + s * (sb[5] + s * (sb[6] + s))))));
    } else {  /* |x| >= 6.666 */
      if (se & 0x8000) return two - tiny;  /* x < -6.666 */
      R = rc[0] + s * (rc[1] + s * (rc[2] + s * (rc[3] + s * (rc[4] + s * rc[5]))));
      S = sc[0] + s * (sc[1] + s * (sc[2] + s * (sc[3] + s * (sc[4] + s))));
    }
    const auto x_words = get_ldouble_words(x);
    const long double z = set_ldouble_words(get<0>(x_words), get<1>(x_words) & 0xffffff00, 0);
    const long double r = expl(-z * z - 0.5625) * expl((z - x) * (z + x) + R / S);
    if ((se & 0x8000) == 0)
      return r / x;
    else
