...
long double coshl(long double x) noexcept {
  static constexpr long double one = 1.0, half=0.5, huge = 1.0e4900L;

  /* High word of |x|. */
  int32_t ex;
  uint32_t mx, lx;
  tie(ex, mx, lx) = get_ldouble_words(x);
  ex &= 0x7fff;
  ...
  if (ex < 0x4003 || (ex == 0x4003 && mx < 0xb0000000u)) {
    const long double t = expl(fabsl(x));
  ...
  if (ex == 0x400c &&
      (mx < 0xb174ddc0u || (mx == 0xb174ddc0u && lx < 0x31aec0ebu))) {
    const long double w = expl(half * fabsl(x));
    const long double t = half * w;
    return t * w;
  }
