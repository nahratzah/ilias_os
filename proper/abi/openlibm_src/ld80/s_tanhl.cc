...
long double tanhl(long double x) noexcept {
  ...
  static constexpr long double two=2.0;
  static constexpr long double tiny = 1.0e-4900L;

  /* High word of |x|. */
  int32_t se;
  uint32_t jj0, jj1;
  tie(se, jj0, jj1) = get_ldouble_words(x);
  const uint32_t ix = se & 0x7fff;
  ...
  if (ix < 0x4003 || (ix == 0x4003 && jj0 < 0xb8000000u)) {  /* |x|<23 */
    ...
    if (ix >= 0x3fff) {	/* |x|>=1  */
      const long double t = expm1l(two * fabsl(x));
      z = one - two / (t + two);
    } else {
      const long double t = expm1l(-two * fabsl(x));
      z= -t / (t + two);
