...
long double log1pl(long double xm1) noexcept {
  ...
  static constexpr array<long double, 6> Q{{
...
int e;
  ...
  if (xm1 == 0.0L) return xm1;

  long double x = xm1 + 1.0L;
  ...
  if ((e > 2) || (e < -2)) {
    ...
    x = z / y;
    z = x * x;
    z = x * (z * __polevll(z, R.begin(), R.end()) /
             __p1evll(z, S.begin(), S.end()));
    z = z + e * C2;
    z = z + x;
    z = z + e * C1;
    return z;
  ...
  long double z = x * x;
  long double y = x * (z * __polevll(x, P.begin(), P.end()) /
