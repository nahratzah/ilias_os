...
struct Elf32_Ehdr {
  static_assert(sizeof(array<unsigned char, EI_NIDENT>) == EI_NIDENT,
                "std::array is broken");

  array<unsigned char, EI_NIDENT> e_ident;
  Elf32_Half e_type;
...
class elf_addr {  // XXX
 public:
  static elf_addr from_elf_addr(types::Elf32_Addr) noexcept;  // XXX
};

class elf_off {  // XXX
 public:
  static elf_off from_elf_off(types::Elf32_Off) noexcept;  // XXX
};

class elf_hdr_flags {  // XXX
 public:
  static elf_hdr_flags from_elf_flags(types::Elf32_Word) noexcept;  // XXX
};
...
class elf_header {
 public:
  ...
  elf_addr get_entry() const noexcept;
  elf_off get_phoff() const noexcept;
  elf_off get_shoff() const noexcept;
  ...
  bool is_class_32() const noexcept;  // 32-bit object
  bool is_class_64() const noexcept;  // 64-bit object
...
class elf_section {
 public:
  ...
  size_t get_entsize() const noexcept;
